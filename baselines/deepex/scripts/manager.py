...
if __name__=="__main__":
    ...
    if args.stage<=3 and (args.stage==3 or not args.debug) and (args.task_meta in ['OIE_2016','WEB','NYT','PENN']):
        RESULT = "result/" + ".".join([args.task,args.model,args.ner_mode,f"d{args.max_distance}",f"b{args.beam_size}"])
        # Get K value from environment variable, default to None (use task-specific defaults)
        k_value = os.environ.get('DEEPEX_K_VALUE')
        k_arg = f" -k {k_value}" if k_value else ""
        SysCall(
            "python3 scripts/oie/evaluate_oie.py -dir {0} -task {1}{2}".format(RESULT+".sorted/",args.task,k_arg)
        )
