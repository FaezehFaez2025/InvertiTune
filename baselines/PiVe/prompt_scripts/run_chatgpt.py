...
output_file_path = f"{iteration_dir}/test_generated_graphs.txt"
...
if args.iteration == 1:
    ...
    prev_iteration = args.iteration - 1
    missing_triples = []
    with open(f'{args.output_dir}/Iteration{prev_iteration}/verifier_result.txt', 'r') as f:
        for l in f.readlines():
            missing_triples.append(l.strip())

    results_old = []
    with open(f"{args.output_dir}/Iteration{prev_iteration}/test_generated_graphs.txt", 'r') as f:
